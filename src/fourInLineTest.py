...
if __name__ == "__main__":
    ...
    # y entrene a q ahora los hago competir
    p1.set_epsilon(0.0)
    p1.set_alpha(0.0)
    #p1.set_gamma(0.0)
    p2.set_epsilon(0.0)
    ...
    #p2.set_gamma(0.0)
    total = 10000
    print "Q tiene " + str(len(p1.q.keys())) + " estados"
    print "Empiezan a competir Q vs r"
    res = [0,0,0]
    for i in range(total):
        t = FourInLine(p1, p2, size[0], size[1])
        ganador = t.play_game(False)
        res[int(ganador)] +=1

    print res

    print "Empiezan a competir r vs Q"
    res = [0,0,0]
    print "Q tiene " + str(len(p1.q.keys())) + " estados"
    for i in range(total):
        t = FourInLine(p2, p1, size[0], size[1])
        ganador = t.play_game(False)
        res[int(ganador)] +=1
